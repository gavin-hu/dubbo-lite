...
public class TelnetCodecTest {
    ...
    private void testDecode_PersonWithEnterByte(byte[] enterbytes ,boolean isNeedmore) throws IOException{
        ...
        Object obj = codec.decode(channel, buffer);
        if (isNeedmore){
            Assert.assertEquals(Codec.DecodeResult.NEED_MORE_INPUT , obj);
        }else {
    ...
    public void testDecode_BlankMessage() throws IOException{
        testDecode_assertEquals(new byte[]{}, Codec.DecodeResult.NEED_MORE_INPUT);
    }
    
    @Test
    public void testDecode_String_NoEnter() throws IOException{
        testDecode_assertEquals("aaa", Codec.DecodeResult.NEED_MORE_INPUT);
    }
    ...
    public void testDecode_String_MiddleWithEnter() throws IOException{
        testDecode_assertEquals("aaa\r\naaa", Codec.DecodeResult.NEED_MORE_INPUT);
    }
    
    @Test
    public void testDecode_Person_ObjectOnly() throws IOException{
        testDecode_assertEquals(new Person(), Codec.DecodeResult.NEED_MORE_INPUT);
    }
    @Test
    ...
    public void testDecode_Backspace() throws IOException{
        //32 8 先加空格在补退格.
        testDecode_assertEquals(new byte[]{'\b'}, Codec.DecodeResult.NEED_MORE_INPUT, new String(new byte[] {32, 8}));
        
        //测试中文
        byte[] chineseBytes = "中".getBytes();
        byte[] request = join(chineseBytes, new byte[]{'\b'});
        testDecode_assertEquals(request, Codec.DecodeResult.NEED_MORE_INPUT, new String(new byte[] {32, 32, 8, 8}));
        //中文会带来此问题 (-数判断) 忽略此问题，退格键只有在真的telnet程序中才输入有意义.
        testDecode_assertEquals(new byte[]{'a', 'x', -1, 'x', '\b'}, Codec.DecodeResult.NEED_MORE_INPUT, new String(new byte[] {32, 32, 8, 8}));
    }
    ...
    public void testDecode_History_UP() throws IOException{
        ...
        testDecode_assertEquals(channel, UP, Codec.DecodeResult.NEED_MORE_INPUT, null);
        ...
        testDecode_assertEquals(channel, UP, Codec.DecodeResult.NEED_MORE_INPUT, expected1);
    }
